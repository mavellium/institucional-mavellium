...
export default function FlippingCardDemo({ title, description }: FlippingCardDemoProps) {
  return (
    ...
    <section className="flex flex-col items-center py-12 md:py-24 px-4 bg-black min-h-screen text-white">
...
function GenericCardFront({ data }: { data: CardData["front"] }) {
  return (
    <div className="flex flex-col h-full w-full overflow-hidden bg-neutral-950 rounded-2xl border border-neutral-800 shadow-2xl transition-all duration-300 group-hover:border-neutral-600">
      ...
      <div className="relative h-2/5 md:h-1/2 overflow-hidden">
        ...
        />
        <div className="absolute top-4 right-4 p-2 bg-black/80 backdrop-blur rounded-full md:hidden border border-neutral-700">
          <RotateCw className="w-4 h-4 text-neutral-400 animate-pulse" />
        </div>
      </div>
      ...
      <div className="p-6 md:p-8 flex flex-col flex-grow">
        ...
        <div className="mt-auto flex items-center text-[10px] md:text-xs font-bold text-neutral-300 uppercase tracking-widest">
          <span className="hidden md:inline">Passe o mouse</span>
          <span className="md:inline lg:hidden">Toque para ver</span>
...
function GenericCardBack({ data }: { data: CardData["back"] }) {
  return (
    <div className="flex flex-col items-center justify-center h-full w-full p-8 md:p-12 text-center bg-neutral-900 rounded-2xl border-2 border-neutral-800 shadow-inner">
      
      <div className="w-16 h-16 rounded-2xl bg-black border border-neutral-700 flex items-center justify-center mb-6 -rotate-3 group-hover:rotate-0 transition-all duration-500">
        <Code className="w-8 h-8 text-white" />
      </div>

      <p className="text-sm md:text-lg leading-relaxed text-neutral-300 mb-8 font-medium">
        {data.description}
      </p>

      <button 
        onClick={(e) => e.stopPropagation()} 
        className="w-full md:w-auto bg-white text-black px-10 py-3 rounded-xl text-sm font-bold shadow-lg transition-all hover:bg-neutral-200 active:scale-95"
      >
        {data.buttonText}
      </button>

      <span className="mt-4 text-[10px] text-neutral-500 uppercase tracking-tighter md:hidden">
