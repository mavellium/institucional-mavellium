...
function PricingCard({ plan }: { plan: PricingCardProps }) {
  return (
    <div
      className={cn(
        "flex flex-col rounded-lg border p-6 text-left border-white/10",
        plan.featured && "border-white shadow-sm ring-1 ring-white/10"
      )}
      aria-label={`${plan.title} plan`}
    >
      <div className="text-center">
        <div className="inline-flex items-center gap-2">
          <Badge variant={plan.featured ? "default" : "secondary"} className="bg-white hover:bg-white/50">{plan.title}</Badge>
          {plan.featured && (
            <span className="rounded-full bg-white/10 px-2 py-0.5 text-xs font-medium text-white">Most popular</span>
          )}
        </div>
        <h4 className="mb-2 mt-4 text-2xl text-white">{plan.price}</h4>
        {plan.description && <p className="text-sm opacity-70 text-white">{plan.description}</p>}
      </div>
      ...
      <ul className="space-y-3">
        {plan.features.map((feature) => (
          <li key={feature} className="flex items-center text-sm opacity-80 text-white">
      ...
      <div className="mt-auto pt-6">
        <Link href={plan.href} target="_blank" rel="noreferrer noopener">
          <Button size="sm" className="w-full bg-white text-black hover:bg-white/50 cursor-pointer" variant={plan.featured ? "default" : "secondary"}>
            {plan.cta}
          </Button>
        </Link>
      </div>
