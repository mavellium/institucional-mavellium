...
export interface FinalCtaData {
  ...
  theme: {
    bg_color?: string;          // opcional, pode ser aplicado no section
    gold_start: string;
    gold_end: string;
    button_bg: string;
  };
...
export default function FinalCtaSection({ data, className = "" }: FinalCtaSectionProps) {
  ...
  // Se necessário, aplicar a cor de fundo da section a partir do tema
  const bgStyle = theme.bg_color ? { backgroundColor: theme.bg_color } : {};

  return (
    <section
      className={`py-32 px-6 relative overflow-hidden flex flex-col items-center justify-center ${className}`}
      style={bgStyle}
    >
      {/* --- AMBIÊNCIA ORIGINAL --- */}
      ...
      <div className="relative z-10 max-w-4xl w-full text-center space-y-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-8 pt-6">
          <Link href={calls_to_action.primary.href} className="group relative">
            ...
            >
              {/* LAYER DE PREENCHIMENTO DOURADO: inicia fora do botão e sobe no hover */}
              <div
              ...
              {/* Texto: muda de branco para preto no hover */}
              ...
              {/* Ícone: fundo escurece no hover */}
              <div className="relative z-10 w-8 h-8 rounded-full bg-white/10 group-hover:bg-black/10 flex items-center justify-center transition-all duration-500 group-hover:rotate-[-50deg]">
                <Icon
